...
export default function HostOnboardingPage() {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      setBeds(user.user_metadata?.beds ?? '');
      setLoading(false);
    };
      ...
      <main className="mx-auto mt-16 flex w-full max-w-2xl flex-col gap-6 px-6">
        <div className="glass flex flex-col gap-6 px-8 py-10">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold text-fg">Расскажите о своём месте</h1>
            <p className="text-sm text-fg/70">Эти детали помогут гостям понять, подходит ли им ваш дом.</p>
          </div>
          <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
            ...
            <label className="flex flex-col gap-2 text-sm text-fg/80">
              ...
              <textarea
                ...
                rows={4}
                placeholder="Что вы любите в своём доме и как принимаете гостей?"
